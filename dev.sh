...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development avalita npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop avalita
    docker rm avalita
    docker run --name avalita -d --env-file /home/ubuntu/avalita.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/avalita:/dkdata \
        avalita start_web.sh
}

function deploy_prod {
  rsync -av --exclude frontend/node_modules --exclude frontend/.nuxt * ubuntu@$HOST_PROD:./avalita/
...
echo_green "Welcome to avalita's dev env"
