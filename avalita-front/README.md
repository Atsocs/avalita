...
```bash
dk bash  # starts bash inside "avalita" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and avalita.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `avalita` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The avalita container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv avalita -p python3  # creates a python3 virtualenv
...
Install docker and nginx. Create an empty postgres database avalita owned by a user avalita.

On your remote machine, create a file ~/avalita.env:

```
DJANGO_DB_PASSWORD=<avalita's password>
DJANGO_DB_HOST=<database_ip>
DJANGO_DB_NAME=avalita
DJANGO_DB_USER=avalita
DJANGO_DEBUG=0
```
...
```
server {
    server_name  avalita.example.com;
    ...
    location /static {
        alias /home/ubuntu/dkdata/avalita/static;
...
(Replace "avalita.example.com" with your production domain)
